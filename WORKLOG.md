...
**What changed**: Kept GitHub `main` as the editable source of truth for the amesvt.com utility landing page and moved production deployment to Cloudflare Pages. The build uploads and rewrites website imagery to the dedicated `ames-website-images` R2 bucket under the `amesvt/` prefix, served through `assets.amesvt.com`. Repository workflows use encrypted, scoped GitHub Actions secrets, with canonical credentials retained in 1Password.
